...
en_model = ngram_model_gene(3, tokens, "EN")
# French
file = open('FR.txt', "r", encoding="MacRoman")
file_cont = file.readlines()
tokens = [word_tokenize(line) for line in file_cont]
fr_model = ngram_model_gene(3, tokens, "FR")

# German
file = open('GR.txt', "r", encoding="MacRoman")
file_cont = file.readlines()
tokens = [word_tokenize(line) for line in file_cont]
gr_model = ngram_model_gene(3, tokens, "GR")
...
for line in sentences:
    ...
    lang_probability = calc_prob(lang_models, ngram_dict_gene(3, words), 3, True)
...
output_file = open('TrigramWordLangId.out', 'w')
output_file.write("ID LANG\n")
for i in range(len(results)):
    output_file.write("%s. %s\n" % (i + 1, results[i]))
output_file.write('\n Percent Correct: %s' % int(acc * 100))
print("results have been wrote in TrigramWordLangId.out file")
output_file.close()
