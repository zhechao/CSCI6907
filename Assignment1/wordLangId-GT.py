...
for line in sentences:
    ...
    lang_probability = calc_prob(lang_models, ngram_dict_gene(2, words), 2, True)
...
output_file = open('WordLangId-GT.out', 'w')
output_file.write("ID LANG\n")
for i in range(len(results)):
    output_file.write("%s. %s\n" % (i + 1, results[i]))
print("results have been wrote in WordLangId-GT.out file")
